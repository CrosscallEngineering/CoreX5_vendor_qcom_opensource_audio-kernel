...
static int wcd937x_init_reg(struct snd_soc_codec *codec)
{
	...
	if (snd_soc_read(codec, WCD937X_DIGITAL_EFUSE_REG_16)
		== 0x01)
				...
				void *data)
{
	...
	case BOLERO_WCD_EVT_SSR_DOWN:
		...
		adc1_vol=snd_soc_read(codec, WCD937X_ANA_TX_CH1);
		adc2_vol=snd_soc_read(codec, WCD937X_ANA_TX_CH2);
		adc3_vol=snd_soc_read(codec, WCD937X_ANA_TX_CH3);
	...
	case BOLERO_WCD_EVT_SSR_UP:
		...
		snd_soc_write(codec, WCD937X_ANA_TX_CH1, adc1_vol);
		snd_soc_write(codec, WCD937X_ANA_TX_CH2, adc2_vol);
		snd_soc_write(codec, WCD937X_ANA_TX_CH3, adc3_vol);
		/*hmct add end*/
		break;
	default:
					  ...
					  int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
	 ...
	 /*** BSP : added by hmct to set  micbias1 2.8v @{ */
         ...
         {
            snd_soc_update_bits(codec, WCD937X_ANA_BIAS, 0x40, 0x40);   
			snd_soc_update_bits(codec, WCD937X_ANA_MICB1, 0x3F, 0x24);   
          }
		 if (micb_num == MIC_BIAS_3) { 
		 	snd_soc_update_bits(codec, WCD937X_ANA_BIAS, 0x40, 0x40);   
			snd_soc_update_bits(codec, WCD937X_ANA_MICB3, 0x3F, 0x24);  
		 	}
